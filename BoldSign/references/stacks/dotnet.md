...
| Base URL (prod) | `https://na1.docusign.net/restapi` (varies by region) | `https://api.boldsign.com` (US) / `https://api-eu.boldsign.com` (EU) |
...
2. Detect the authentication method used in the Docusign code. If the user uses an accessToken or Bearer token, you must use BoldSign OAuth. Do not switch to an API Key unless the user explicitly mentions to use API key.
